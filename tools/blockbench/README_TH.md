...
4. เปิด `.bbmodel` ตั้งชื่อโปรเจกต์ แล้วสร้างโมเดลได้เลย

## วิธีใช้แบบสั้น

1. ใช้ Profile เริ่มต้น `Accessory` สำหรับงานแรก; เปลี่ยนเป็น `Full Body` หรือ `Custom` เมื่อจำเป็น
...
ไม่ต้องเขียน `avatar.json` เอง Plugin ใช้ชื่อโปรเจกต์เป็นชื่อ Avatar และสร้างไฟล์ให้ใน ZIP ช่อง Shyne Avatar Name กับ ID เว้นว่างได้ ชื่อภาษาไทยใช้ได้และจะได้ ID ที่ปลอดภัยโดยอัตโนมัติ
...
หากต้องการ Lua ให้เลือก `File → Export → Export Shyne Lua Starter (optional)` ก่อน Plugin จะสร้าง `script.lua` เริ่มต้นและเปิดตัวเลือก Lua ในโปรเจกต์ปัจจุบัน จากนั้นแก้สคริปต์และเลือกไฟล์นี้ตอน Export Package
