...
public final class ShyneItemRuntime {
    ...
    public ItemStack createStack(String itemId, int count) {
        ...
        ItemStack stack = new ItemStack(ShyneItems.ARTIFACT.get(), Math.min(Math.max(1, count), definition.maxStack()));
    ...
    public ShyneItemDefinition resolve(ItemStack stack) {
        if (stack == null || stack.isEmpty() || stack.getItem() != ShyneItems.ARTIFACT.get()) return null;
