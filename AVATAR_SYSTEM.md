...
ปุ่ม `ผู้เล่น` ใน Avatar Library เปิดรายการผู้เล่นที่เชื่อมต่อ Shyne บนเซิร์ฟเวอร์ปัจจุบัน พร้อมชื่อ Avatar และระดับภาระโมเดลโดยประมาณ ผู้ใช้กำหนดนโยบายแยกตาม UUID ได้ดังนี้:

- `ซ่อน` หยุดวาด Avatar และคืน vanilla skin/nameplate โดยยังเก็บ snapshot ไว้เพื่อเปิดกลับได้ทันที
- `ป้ายชื่อ` ปิด custom nameplate ของ Avatar โดยใช้ร่วมกับการซ่อนได้ ปัจจุบัน Shyne ไม่รัน Lua, sound, particle, HUD หรือ world-render task ของ peer บนเครื่องผู้รับอยู่แล้ว
- `บล็อก` ลบ model, texture, animation, part state และ synced vars ของผู้เล่นนั้นออกจาก client และปฏิเสธ snapshot ใหม่
- `ซ่อนทั้งหมด` คืนผู้เล่นระยะไกลทั้งหมดเป็น vanilla โดยไม่กระทบ Avatar ของตัวเอง

การเลือกทั้งหมดเป็น local-only เก็บใน `config/shyne-creator-client.json` ตาม UUID และไม่แจ้งอีกฝ่าย Avatar จาก peer ที่ไม่มี metadata การจัดเรตจะแสดงเป็น `ยังไม่จัดเรต`; Shyne ไม่อ้างว่าสามารถตรวจ NSFW จากโมเดลในเครื่องได้อย่างสมบูรณ์
