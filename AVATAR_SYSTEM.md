...
```json
{
  ...
  "permissions": ["particle", "sound", "camera"]
}
```

`id` ใช้ตัวพิมพ์เล็ก ตัวเลข `_` `-` หรือ `.` และต้องไม่ซ้ำกับอวตารอื่น

`permissions` ใช้กับ Public Avatar เพื่อประกาศความสามารถพิเศษที่ Lua ต้องใช้ รองรับ `particle`, `sound`, `camera`, `microphone` และ `command` เท่านั้น หากไม่ประกาศ runtime จะปฏิเสธ API นั้น ผู้ใช้จะเห็นหน้าตรวจสิทธิ์ก่อนเปิดแพ็กเกจ โดย `camera`, `microphone` และ `command` ปิดไว้เป็นค่าเริ่มต้น การอนุมัติผูกกับ `share_id + package_hash` จึงต้องอนุมัติใหม่เมื่อผู้สร้างอัปเดตแพ็กเกจ ส่วน Avatar ในเครื่องยังทำงานตามปกติโดยไม่แสดงหน้าขอสิทธิ์
...
ไม่มี alias ของ API รุ่นทดลองก่อนหน้า แพ็กที่ไม่ระบุ `api_version` จะถูกล็อกเป็น Standard 1 โดยอัตโนมัติ ส่วนแพ็กที่ระบุเลขรุ่นเองต้องเป็นรุ่นที่รองรับ จึงยังตรวจจับการเปิดแพ็กด้วย API ผิดรุ่นได้
