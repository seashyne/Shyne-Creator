...
Shyne Creator 2.7.33 มีเครื่องมือสร้างโครงโปรเจกต์ ตรวจไฟล์ ดูประสิทธิภาพ และ Shyne Lua API Standard 1.1 ในเกม โดย Avatar ยังคงเป็นไฟล์ภายนอกและไม่ถูกฝังในตัว Mod
...
เปิด `model.bbmodel` ด้วย Blockbench แล้วแก้ `script.lua` ได้ทันที Texture จะถูกค้นหาอัตโนมัติ ไม่บังคับชื่อไฟล์ `avatar.json` ต้องมีเพียง `name` และจะเลือก API ล่าสุด, `id`, `main`, `model` รวมถึงค่าแนะนำอื่นให้อัตโนมัติ โปรเจกต์ใหม่จากเครื่องมือจะใส่ `api: "latest"` และ `requires` ให้เห็นชัดเจน
...
ตัวอย่างพร้อมใช้: `tools/examples/lua-api-1.1-avatar`, `tools/examples/render-profiler-avatar`, `tools/examples/advanced-render-avatar` และ `tools/examples/responsive-hud-avatar`
