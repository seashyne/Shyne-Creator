...
public final class CloudAvatarLibraryScreen extends Screen {
    ...
    @Override
    protected void init() {
        panelWidth = Math.min(760, this.width - 20);
        ...
        for (int i = 0; i < Math.min(PAGE_SIZE, items.size()); i++) {
            final int index = i;
        ...
        Button previous = Button.builder(Component.literal("‹"), button -> { offset = Math.max(0, offset - 30); refresh(); })
            .bounds(listX, panelY + 336, 32, 20).build();
        ...
        Button next = Button.builder(Component.literal("›"), button -> { if (nextOffset != null) { offset = nextOffset; refresh(); } })
            .bounds(listX + 36, panelY + 336, 32, 20).build();
        ...
        Button download = Button.builder(Component.translatable(publicMode ? "screen.shyne_core.cloud.use_public" : "screen.shyne_core.cloud.restore"), button -> downloadCurrent())
            ...
            .bounds(detailX, panelY + 224, detailWidth, 20).build();
        ...
        if (publicMode && current != null && !current.permissions().isEmpty()) {
            addRenderableWidget(Button.builder(Component.translatable("screen.shyne_core.cloud.permissions"), button -> openPermissions(current))
                ...
                .bounds(detailX, panelY + 248, detailWidth, 20).build());
        }
        ...
        Button upload = Button.builder(Component.translatable("screen.shyne_core.cloud.backup_active"), button -> uploadActive())
            ...
            .bounds(detailX, panelY + (publicMode && current != null && !current.permissions().isEmpty() ? 272 : publicMode || current == null ? 248 : 272), detailWidth, 20).build();
        ...
        addRenderableWidget(Button.builder(Component.translatable("screen.shyne_core.settings"), button -> {
            ...
            .bounds(panelX + panelWidth - 280, panelY + 358, 86, 20).build());
        if (loading && operation.cancellable()) {
            addRenderableWidget(Button.builder(Component.translatable("screen.shyne_core.cloud.cancel"), button -> cancelOperation())
                ...
                .bounds(panelX + panelWidth - 184, panelY + 358, 86, 20).build());
        ...
        addRenderableWidget(Button.builder(Component.translatable("gui.back"), button -> onClose())
            .bounds(panelX + panelWidth - 94, panelY + 358, 80, 20).build());
    ...
    private void refresh() {
        ...
        request.whenComplete((page, error) -> Minecraft.getInstance().execute(() -> {
            loading = false;
    ...
    private void signIn() {
        if (ShyneCloudClient.signedIn()) {
            ...
            rebuildWidgets(); if (publicMode) refresh(); return;
        }
        loading = true;
        retryAction = this::signIn;
        var request = ShyneCloudClient.signIn(Minecraft.getInstance());
        activeRequest = request;
        request.whenComplete((result, error) -> Minecraft.getInstance().execute(() -> {
            loading = false; rebuildWidgets(); if (result != null && result.state() == ShyneCloudClient.State.SUCCESS) refresh();
        }));
        rebuildWidgets();
    }
    ...
    private void startDownload(ShyneCloudClient.CloudAvatar current) {
        ...
        activeRequest.whenComplete((result, error) -> Minecraft.getInstance().execute(() -> {
            loading = false; AvatarRuntime.refreshCatalog(); rebuildWidgets();
        }));
        rebuildWidgets();
    }
    ...
    private void uploadActive() {
        ...
        request.whenComplete((result, error) -> Minecraft.getInstance().execute(() -> {
            ...
            if (result != null && result.state() == ShyneCloudClient.State.SUCCESS) {
                offset = 0; items = List.of(); selected = 0; rebuildWidgets(); refresh();
            } else rebuildWidgets();
        }));
        rebuildWidgets();
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        graphics.fillGradient(panelX, panelY, panelX + panelWidth, panelY + PANEL_HEIGHT, 0xF20A1630, 0xF2071026);
        graphics.outline(panelX, panelY, panelWidth, PANEL_HEIGHT, 0xFF22D7E8);
        ...
        graphics.fill(divider, panelY + 76, divider + 1, panelY + 330, 0x5532D8EA);
        ShyneCloudClient.CloudAvatar current = selectedAvatar();
        ...
        if (operation.state() == ShyneCloudClient.State.WORKING) {
            ...
            graphics.fill(barLeft, panelY + 350, barRight, panelY + 353, 0x4432D8EA);
            graphics.fill(barLeft, panelY + 350, filled, panelY + 353, 0xFF32D8EA);
        }
        graphics.text(this.font, Component.literal(this.font.plainSubstrByWidth(operation.message(), panelWidth - 220)), panelX + 14, panelY + 364, color, false);
